...
class particleFilter:
	...
	def __init__(self, dimension = 2, model = default_2D_Model(), numParticles = numberOfParticles, timeSpan = timsSpan, resamplingNoise = 0.01, positionStd = 5):
        ...
        # Student finish the code below
	        ...
	        self.weights = np.ones(self.numParticles)
	...
	def Measurement_Model(self, x, y):
        ...
        ####################################################################
	        position = self.model.readingSensor(x, y)

	        weight_particle = np.zeros((self.numParticles, 1))

	        for i in range(self.numParticles):
	        	distance = np.array(self.model.readingMap(np.array(self.particles[i])))
	        	rel_distance =  np.linalg.norm(distance - position)

	        	weight = np.exp(-1*(rel_distance**2) / (2*self.std))
	        	weight_particle[i] = weight

	        	sum_weights = sum(weight_particle)

	        if sum_weights == 0:
	        	print("Sum of weights is 0")
	        	self.weights = np.ones(self.numParticles) * float (1 / self.numParticles)
	        else:
	        	self.weights = weight_particle / sum_weights
	...
	def calcPosition(self):
        ...
        # Student finish teh code below
	        ...
	        weights = sum(np.multiply(self.particles, self.weights))

	        x = max(min(weights[0], self.curMax[0]), self.curMin[0])
	        y = max(min(weights[1], self.curMax[1]), self.curMin[1])
